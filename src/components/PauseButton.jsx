...
const PauseButton = ({ onClick, size = 'large' }) => {
  ...
  const topPathId = `topPathPause-${buttonId}`;
  const bottomPathId = `bottomPathPause-${buttonId}`;
  const darkGradientId = `darkGradientPause-${buttonId}`;
  const subtleGreenId = `subtleGreenPause-${buttonId}`;
  const pauseGradientId = `pauseGradientPause-${buttonId}`;
  const circleClipId = `circleClipPause-${buttonId}`;
  const outerGlowId = `outerGlowPause-${buttonId}`;
    ...
    >
      ...
      <svg viewBox="0 0 168 168" xmlns="http://www.w3.org/2000/svg" className="w-full h-full relative z-10">
        <defs>
          ...
          <linearGradient id={pauseGradientId} x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        {/* Icono de pausa en el centro con ondas de Spotify */}
        <g>
          ...
          {/* Dos barras verticales de pausa */}
          <rect x="74" y="72" width="8" height="24" rx="2" fill={`url(#${pauseGradientId})`} />
          <rect x="86" y="72" width="8" height="24" rx="2" fill={`url(#${pauseGradientId})`} />
        </g>
      </svg>
    </button>
  );
